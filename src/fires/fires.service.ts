...
export class FiresService {
  async getFireData(date: string): Promise<any> {
    const url = `https://firms.modaps.eosdis.nasa.gov/api/area/csv/5d7359a6d0e9d5adc5546281ebcbe071/VIIRS_SNPP_NRT/world/1/${date}`;
    ...
    const topFireData = jsonData.filter((fire, idx) => idx % 100 === 0);
    // .sort((a, b) => parseFloat(b.bright_ti4) - parseFloat(a.bright_ti4))
    // .slice(0, 1000); // Select top 100
